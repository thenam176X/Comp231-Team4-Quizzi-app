...
const TakeQuizPage = (props) => {
    ...
    const timer = setInterval(() => {
      setTimeLeft((timeLeft) => {
        if (timeLeft === 0) {
          if (!isAnswered) {
            alert(
              "Time is up! Moving to the next question. This question will be marked as incomplete."
            );
          }
          handleNextQuestion();
          return null;
  ...
  const handleNextQuestion = () => {
    if (
      quiz.questions[currentQuestionIndex].quizType === "fill-in-the-blank" &&
    ...
    setFillInTheBlankAnswerArray([
      ...fillInTheBlankAnswerArray,
      fillInTheBlankAnswer,
    ]);
  ...
  const handleSubmitQuiz = () => {
    ...
    alert(
      `You have completed ${completedQuestions} questions and ${incompleteQuestions} questions are marked as incomplete.`
    );
    ...
    <div style={styles.container}>
      ...
      {question.quizType === "fill-in-the-blank" ? (
        <input
          ...
          onChange={(e) =>
            setFillInTheBlankAnswer(e.target.value.toUpperCase())
          }
